...
def create_likelihood_function_mk(tree, chars, Qtype, pi="Equal"):
    ...
    def likelihood_function(Qparams):
        ...
        try:
            return -1 * mk(tree, chars, var["Q"], p=var["p"], pi = pi, preallocated_arrays=var) # Minimizing negative log-likelihood
    ...
    if type(Q) == str:
        if Q == "Equal":
            ...
            # to determine likelihood
                ...
                pivals = mk(tree, chars, q, pi="Fitzjohn", returnPi=True)[1]
                q,l,piRates = fitMkER(tree, chars, pi=pivals)
                return {key:val for key, val in zip(["Q", "Log-likelihood","pi"], [q,l,piRates])}
