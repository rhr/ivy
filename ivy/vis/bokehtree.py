...
class BokehTree(object):
	def __init__(self, root, scaled = True, nodelabels = True, tiplabels = True, showplot = True, hover = False):
		...
		self.tools = [WheelZoomTool(),BoxZoomTool(),ResizeTool(),ResetTool(),PanTool(),PreviewSaveTool()]
		self.source = None # Data source for plotting node points. Allows hovertool to work.
		...
		for node in self.root.postiter():
			if node.children:
				...
				for i in node.children:
					...
					# Storing coordinates
					self.xLineCoords.extend([[nodeParx, nodeParx], [nodeParx, nodeChix]])	
					self.yLineCoords.extend([[nodeChiy, nodePary], [nodeChiy, nodeChiy]])
	...
	def drawlabels(self):
		...
		if self.nodelabels:
			# Drawing tip labels
			self.plot.text([ k.xval + 0.02*self.plot_x_range for k in self.root.leaves() ],
		...
		if self.tiplabels:
			# Drawing node labels
			self.plot.text([ k.xval + 0.02*self.plot_x_range for k in self.root.clades() ],
	...
	def drawtree(self, showplot = True):
		...
		self.plot = figure(plot_width = 400, plot_height = 400, tools=self.tools)
		...
		if self.tiplabels == True:
			self.plot_x_range = (max([ i[1] for i in self.coords ]) - min([ i[1] for i in self.coords ]))
			self.plot_y_range = (max([ i[0] for i in self.coords ]) - min([ i[0] for i in self.coords ]))
	
			self.plot.x_range = Range1d( (min([ i[1] for i in self.coords ]) - (0.2*self.plot_x_range)), 
										 (max([ i[1] for i in self.coords ]) + (0.2*self.plot_x_range)))
		...
		# Trimming x axis
		self.plot.xaxis.bounds = (min([ i[1] for i in self.coords ]), max([ i[1] for i in self.coords ]))
